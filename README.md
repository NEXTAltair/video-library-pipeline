...
- `src/plugin/`
  - Openclaw plugin 登録、CLI、plugin hook
- `src/tools/`
  - `video_pipeline_*` ツールアダプタ
- `src/platform/`
  - 設定解決、ランタイム、PowerShell テンプレート同期
- `src/core/`
  - ツール間で共有する TS 実行ロジック
- `src/contracts/`
  - TypeScript 側の共通型
- `py/*.py`
  - 現行の実行エントリポイントと互換ラッパー
- `py/video_pipeline/domain|db|platform/`
  - Python 共有ロジックの本体
- `templates/windows-scripts/`
  - プラグイン同梱の PowerShell テンプレート本体
- `assets/windows-scripts/`
  - 移行期間中のレガシーフォールバック
...
> ~~**既知の問題:** [#11](https://github.com/NEXTAltair/video-library-pipeline/issues/11) PS1スクリプト統合~~ 統合見送り / ~~[#12](https://github.com/NEXTAltair/video-library-pipeline/issues/12) PSエラーハンドリング改善~~ → 解決済み (`moveApplyStats` で構造化エラー伝播)
...
| 14 | ~~WSL2 ERRNO5 (EIO) スキャン不安定性~~ | [#32](https://github.com/NEXTAltair/video-library-pipeline/issues/32) | 解決済み (`windows_ops_root` 指定時は PowerShell primary scan で 9P バイパス) |
