...
export function buildLlmExtractTask(inputJsonlPath: string, outputJsonlPath: string, hintsPath: string): string {
  return [
    ...
    `Each line is a JSON object with fields: path_id, name (filename), path (Windows path), mtime_utc, epg_hint_titles?, epg_hint_start_at?`,
    "",
    `## Hints`,
    ...
    `- Prefer epg_hint_titles[0] as program_title when filename is ambiguous, but only if it plausibly matches the filename context.`,
    `- Use epg_hint_start_at as a secondary clue for validating air_date when present.`,
    "",
    `## Output`,
