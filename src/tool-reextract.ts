...
export function buildLlmExtractTask(inputJsonlPath: string, outputJsonlPath: string, hintsPath: string): string {
  return [
    ...
    `Each line is a JSON object with fields: path_id, name (filename), path (Windows path), mtime_utc, and optional epg_hints.`,
    `When epg_hints is present, it includes candidates[] with title/air_date/start_time/broadcaster from EPG.`,
    "",
    `## Hints`,
    ...
    "",
    `## Rules`,
    `- If epg_hints.candidates exists, use it as a strong hint for program_title and air_date when filename alone is ambiguous.`,
    `- Prefer EPG candidate titles that closely match filename tokens/date-time.`,
