...
export function buildLlmExtractTask(inputJsonlPath: string, outputJsonlPath: string, hintsPath: string): string {
  return [
    ...
    `Each line is a JSON object with fields: path_id, name (filename), path (Windows path), mtime_utc.`,
    `Some rows may also include optional EPG hints: epg_hint_titles (string[]), epg_hint_broadcast_datetime, epg_hint_broadcaster.`,
    "",
    `## Hints`,
    ...
    `When epg_hint_titles exists, treat it as high-priority candidate titles to disambiguate ambiguous filenames.`,
    `If filename-derived title conflicts with an EPG hint but the filename is ambiguous/truncated, prefer the EPG hint and lower confidence only if still uncertain.`,
    "",
    `## Output`,
