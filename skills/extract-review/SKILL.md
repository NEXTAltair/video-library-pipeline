...
- Use only `video_pipeline_status`, `video_pipeline_inspect_artifact`, and `video_pipeline_resume`.
...
5. After the user confirms review is complete, execute the exact returned follow-up:
   ```json
   video_pipeline_resume {
     "runId": "<runId>",
     "resumeAction": "<resumeAction from nextActions>",
     "...": "other params from followUpToolCalls[].params"
   }
   ```
